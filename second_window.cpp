...
void second_window::insert_data(const purchase_collection* collection) const
{
    ...
    for (int i = 0; i < collection->size();i++)
    {
        ...
        item = new QStandardItem(QString(QString::fromStdString(collection->get_iterator()[i].get_date())));
        model->appendRow(item);
        item = new QStandardItem(QString(QString::fromStdString(collection->get_iterator()[i].get_time())));
        model->appendRow(item);
